...
const int CYCLE_CHUNKING[4] = {16, 0, 16, 0};
const int PHASE_NUM = CYCLE_CHUNKING[0] + CYCLE_CHUNKING[1] + CYCLE_CHUNKING[2] + CYCLE_CHUNKING[3];
const float MAX_STEPSIZE = 12; //设置最大步长
const float DELAY_TIME = 0.002;
const float MAX_UPLENGTH = 5, MAX_DOWNLENGTH = -1;
const float TURN_STEPSIZE = 5;
const float DELTA_STEPSIZE = 0.4;

const float KP_X = 0.1, KD_X = 0, KP_Z = 0.5, KD_Z = 0;
...
void forward(Leg leg[4]){
    ...
    leg[1].set_step(MAX_STEPSIZE + DELTA_STEPSIZE, MAX_UPLENGTH, MAX_DOWNLENGTH);
    leg[3].set_step(MAX_STEPSIZE + DELTA_STEPSIZE, MAX_UPLENGTH, MAX_DOWNLENGTH);
}
...
void turn_right(Leg leg[4]){
    // 右
    leg[0].set_step(-TURN_STEPSIZE, MAX_UPLENGTH, MAX_DOWNLENGTH);
    leg[2].set_step(-TURN_STEPSIZE, MAX_UPLENGTH, MAX_DOWNLENGTH);
    // 左
    leg[1].set_step(TURN_STEPSIZE, MAX_UPLENGTH, MAX_DOWNLENGTH);
    leg[3].set_step(TURN_STEPSIZE, MAX_UPLENGTH, MAX_DOWNLENGTH);
}
...
//     Pd_force Pd_forceControlller(KP_X, KP_Z, KD_X, KD_Z);

//     Leg leg[4] = {
//           Leg(0, R_F, init_pos[0], signals[0], true, CYCLE_CHUNKING, Pd_forceControlller)
//         , Leg(1, L_F, init_pos[1], signals[1], false, CYCLE_CHUNKING, Pd_forceControlller)
//         , Leg(2, R_H, init_pos[2], signals[2], false, CYCLE_CHUNKING, Pd_forceControlller)
//         , Leg(3, L_H, init_pos[3], signals[3], true, CYCLE_CHUNKING, Pd_forceControlller)
//         };
//     int phase;
//     bool p_pressed;
//     while(ros::ok()){
...
int main(int argc, char** argv)
{
    ...
    Leg testLeg(0, R_F, init_pos[0], signals[0], true, CYCLE_CHUNKING, Pd_forceControlller);

    while(ros::ok()){
        ros::spinOnce();
        testLeg.testController();
    }
}
